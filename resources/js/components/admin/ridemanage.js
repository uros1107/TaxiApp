...
const headCells = [
  ...
  { id: 'date', numeric: false, disablePadding: false, label: 'Order Date' },
...
export default function EnhancedTable(props) {
  ...
  const [is_rideEdit, setRideEdit] = React.useState(false);
  const [is_ridePay, setRidePay] = React.useState(false);
  const [rowsPerPage, setRowsPerPage] = React.useState(5);
  const [isLoaded, setIsLoaded] = React.useState(false);

  const { location } = props;
  const previousPath = location.custom_id;
  var passenger_id = previousPath;
  ...
  useEffect(() => {
    ...
    fetch("/api/getallrides", requestOptions)
  ...
  const handleClick = (event, name, history_id) => {
  ...
  const rideEdit = (order_id) => {
    setOrderId(order_id)
    setRideEdit(true)
  }

  const ridepay = (order_id) => {
    setOrderId(order_id)
    setRidePay(true)
  }
  ...
  if(is_rideEdit) {
    return <Redirect to={{ pathname: '/rideedit', order_id: orderid }}  />
  }

  if(is_ridePay) {
    return <Redirect to={{ pathname: '/ridepay', order_id: orderid }}  />
  }
         ...
         <Container>
            ...
            <Row>
                <Col xs={12} className="text-center">
                    <h4 style={headerStyle}>Ride Management</h4>
                </Col>
            </Row>
            {/* <Row>
                        ...
                        return (
                            ...
                            >
                              ...
                              <TableCell align="center">
                                  <Avatar align="center" alt="Remy Sharp" src={row.photo == null? "assets/images/users/default.png" : "assets/images/users/a" + row.photo} className={classes.small} />
                              </TableCell>
                              ...
                              <TableCell align="left">{row.order_time}</TableCell>
                              {row.status == 0 && (
                              ...
                              <TableCell align="center"><EditIcon style={{color: "#6f6f6f"}} onClick={() => rideEdit(row.id)}/></TableCell>
                              <TableCell align="center"><PaymentIcon style={{color: "#6f6f6f"}} onClick={() => ridepay(row.id)}/></TableCell>
                            </TableRow>
                        );
                        })}
                    {emptyRows > 0 && (
